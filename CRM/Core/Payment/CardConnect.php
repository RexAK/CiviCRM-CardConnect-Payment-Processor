<?php
	...
	class CRM_Core_Payment_CardConnect extends CRM_Core_Payment {
		...
		function verifyOrder( $payment_response ) {
			...
			$values = CRM_Core_DAO::executeQuery( $sql, array(
				1 => array(
					1,
					'Int',
				),
		...
		function doDirectPayment( &$params ) {
			...
			if ( !( array_key_exists( 'qfKey', $params ) ) ) {
				...
				$params['cardconnect_error_url'] = $error_url = CRM_Utils_System::url( $url_path, $parsed_url['query'] . "&_qf_Main_display=1&qfKey={$qfKey}", false, NULL, false );
			}
			...
			if ( CRM_Utils_Array::value( 'is_recur', $params ) && $params['contributionRecurID'] ) {
				CRM_Core_DAO::setFieldValue( 'CRM_Contribute_DAO_Contribution', $params['contributionID'], 'contribution_status_id', PENDING_CONTRIBUTION_STATUS_ID );

				CRM_Core_DAO::setFieldValue( 'CRM_Contribute_DAO_ContributionRecur', $params['contributionRecurID'], 'processor_id', $params['month'] . '/' . $params['year'] . '_' . $card_details );

				CRM_Core_DAO::setFieldValue( 'CRM_Contribute_DAO_ContributionRecur', $params['contributionRecurID'], 'create_date', CRM_Utils_Date::isoToMysql( date( 'Y-m-d H:i:s' ) ) );
			} else {
		...
		public function processPendingContributions( $payment_processor ) {
			...
			foreach ( $pending_contributions as $pending_contribution ) {
				...
				CRM_Core_DAO::setFieldValue( 'CRM_Contribute_DAO_ContributionRecur', $pending_contribution['contribution_recur']->id, 'next_sched_contribution_date', CRM_Utils_Date::isoToMysql( $next_sched ) );

				CRM_Core_DAO::setFieldValue( 'CRM_Contribute_DAO_ContributionRecur', $pending_contribution['contribution_recur']->id, 'contribution_status_id', IN_PROGRESS_CONTRIBUTION_STATUS_ID );
			}
		}
		...
		public function processScheduledContributions( $payment_processor ) {
			...
			foreach ( $scheduled_contributions as $contribution ) {
				if ( $contribution->payment_processor_id != $payment_processor['id'] ) continue;

				$next_sched = CRM_Core_DAO::getFieldValue( 'CRM_Contribute_DAO_ContributionRecur', $contribution->id, 'next_sched_contribution_date', 'id', true );
				...
				if ( ( $contribution->installments <= 0 ) || ( $contribution->installments > $ccount + 1 ) ) {
					...
					$next_sched = NULL;
					civicrm_api( 'ContributionRecur', 'create', array(
						'version'                      => '3',
						'id'                           => $contribution->id,
				...
				$addresses = civicrm_api( 'Address', 'get', array(
					'version'    => '3',
				...
				if ( $next_sched ) {
					CRM_Core_DAO::setFieldValue( 'CRM_Contribute_DAO_ContributionRecur', $contribution->id, 'next_sched_contribution_date', CRM_Utils_Date::isoToMysql( $next_sched ) );
				} else {
					CRM_Core_DAO::setFieldValue( 'CRM_Contribute_DAO_ContributionRecur', $contribution->id, 'contribution_status_id', COMPLETED_CONTRIBUTION_STATUS_ID );
					CRM_Core_DAO::setFieldValue( 'CRM_Contribute_DAO_ContributionRecur', $contribution->id, 'end_date', CRM_Utils_Date::isoToMysql( date( 'Y-m-d 00:00:00', time() ) ) );
				}
		...
		function getPendingContributions( $payment_processor ) {
			...
			while ( $pending_contributions->fetch() ) {
				if ( $pending_contributions->contribution_recur_id ) {
					...
					if ( $recurring->find( true ) && $recurring->processor_id && ( $recurring->payment_processor_id == $payment_processor['id'] ) ) {
		...
		function getScheduledContributions( $payment_processor ) {
			...
			while ( $scheduled_contributions->fetch() ) {
				...
				$contribution->whereAdd( "`receive_date` >= '{$scheduled_contributions->next_sched_contribution_date}'" );
		...
		function checkPorts() {
			foreach ( $this->cc_ports as $env => $port ) {
				$settingsURL = str_replace('https://','ssl://',$this->_paymentProcessor['url_site']);
				$settingsURL = str_replace('http://','ssl://',$settingsURL);

				$fp = fsockopen( $settingsURL, $port, $errno, $errstr, 5 );
				if ( !$fp ) {
					$title = 'CardConnect Port or Connectivity issue';
					$message = 'Please contact your hosting provider and insure port '.$port.' is open.';
					CRM_Core_Session::setStatus( $message, $title, 'error' );

				} else {
					// port is open and available
					fclose( $fp );
					$title = 'CardConnect Payment Processor';
					$message = 'Looks like port '.$port.' is open!';
